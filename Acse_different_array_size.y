...
exp: NUMBER      { $$ = create_expression ($1, IMMEDIATE); }
   ...
   | IDENTIFIER {
	   ...
	   if( size < getVariable(program, $1)->arraySize ){ 	
	   	size = getVariable(program, $1)->arraySize;
		}

   } LSQUARE exp RSQUARE {
	int reg;
	/*se $4 supera il max dell array size corrente, salto\proseguo al prossimo incremento*/
	if(exist) { 

		handle_binary_comparison (program, $4, create_expression(getVariable(program, $1)->arraySize, IMMEDIATE), _LT_); gen_beq_instruction(program, begin_increment, 0);
  ...
  | EXISTS list_ids { 
	...
	LPAR exp RPAR {
		...
		// Increment the index
	   	gen_addi_instruction(program, array_size_reg,  REG_0, size);
