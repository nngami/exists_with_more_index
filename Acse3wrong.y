...
+
+ 	if(get_symbol_location(program, $3, 0)){
+
...
+	} | IDENTIFIER {
+ 
+ 	if(get_symbol_location(program, $1, 0)){
+
+		id_exist_info idExistInfo;
...
+			if(exist){
+
+				id_exist_info iei = getLastElement(exist); int found=0;
+			/* get the location of the symbol with the given ID that belong to the current exist's list_ids */
+				for(char *i = ((id_exist_info)LDATA(iei)).id; iei!=NULL; iei = (id_exist_info)LPREV(iei)){
+
+				     if (!strcmp(i, $1)){
+					     location = i.index_reg;
+					     found=1;
+					     break;
+				     } 
+
