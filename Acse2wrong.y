...
+
+	t_axe_label *stop_incremento = newLabel(program), *prosegui_incremento = newLabel(program);
+	int reg, regPrev;
+	t_list *prev = NULL;
...
+		if(prev){
+
+			regPrev = getNewRegister(program);
+			gen_add_instruction(program, regPrev, regPrev, ((id_exist_info)LDATA(prev))->index_reg, 0);
+			gen_addi_instruction(program, regPrev, regPrev, 1);
+			gen_subi_instruction(program, getNewRegister(program), regPrev, 10);
+			gen_blt_instruction(program, prosegui_incremento, 0);
+			p->index_reg = gen_load_immediate(program, 0);
+
+		} 
+
+		assignLabel(program, prosegui_incremento);
+
+		p = p->prev;
+
+	}
