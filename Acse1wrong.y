...
+  	t_axe_expression is_visit_not_completed = handle_binary_comparison (program, create_expression(greater_than_array_size, REGISTER), 		create_expression(1, IMMEDIATE), _NOTEQ_);
+
...
+		int reg = ((id_exist_info)LDATA(p))->index_reg;
...
+		if(prev){
+
+			int regPrev = getNewRegister(program);
