...
exp: NUMBER      { $$ = create_expression ($1, IMMEDIATE); }
   | IDENTIFIER  {printf(" ID ");
			...
			if(exist){printf(" IDexist ");

				t_list *iei = getLastElement(exist); 
				int found=0;
			/* get the location of the symbol with the given ID that belong to the current exist's list_ids */
			char *i = NULL; 
			...
			/* another possible ID in the current exist context that not belong to the current exist's list_ids */
				if(!found){printf(" IDexistfound "); location = get_symbol_location(program, $1, 0);}

			}else {printf(" IDelseexist ");

				   /* get the location of the symbol with the given ID out of an exist context*/
				   location = get_symbol_location(program, $1, 0);
                     ...
                     free($1);
   printf(" ID ");}
   | IDENTIFIER LSQUARE exp RSQUARE {printf(" array ");

           int reg = gen_load_immediate(program, 0);
           t_axe_variable* v = getVariable(program, $1);

		    t_axe_label *greater_than_array_siz = newLabel(program), *no_greater_than_array_siz = newLabel(program);

			if(exist){
		/*se qualche index di exist ha fatto si che exp superi il size del array $1 aloora siamo al limite e dobbiamo fermarci*/
					if($3.expression_type == IMMEDIATE){
		
							if($3.value >= v->arraySize){
								greater_than_array_size = gen_load_immediate(program, 1);
								gen_blt_instruction(program, greater_than_array_siz, 0);
				
							}
		
		 			}else{
		
						gen_subi_instruction(program,getNewRegister(program), greater_than_array_size, v->arraySize);
						gen_blt_instruction(program, no_greater_than_array_siz, 0);
						greater_than_array_size = gen_load_immediate(program, 1);
						gen_blt_instruction(program, greater_than_array_siz, 0);
                     ...
                     * into `arrayElement' */
		     assignLabel(program, no_greater_than_array_siz);
                     reg = loadArrayElement(program, $1, $3);
		     assignLabel(program, greater_than_array_siz);
   ...
   | EXISTS {printf(" EXISTS ");
	/*array_size non è superato*/
	greater_than_array_size = gen_load_immediate(program, 0);

	printf(" EXISTS ");} list_ids { printf(" list_ids ");

 	    $1 = assignNewLabel(program);

  printf(" list_ids ");} LPAR exp RPAR {printf(" exp ");

	// Increment the indexes ( the n-tuple on decimal number 0-9 )
	t_list *p = getLastElement(exist);

	t_axe_label *stop_incremento = newLabel(program), *riprendi_incremento_dal_primo_index = newLabel(program);
	int reg, regPrev;
	t_list *prev = NULL;
	int tutte_le_tuple_non_sono_state_provate = gen_load_immediate(program, 1);

	assignLabel(program, riprendi_incremento_dal_primo_index);	
	while(p){
	/*ho ippotizzato che gli index_reg di ogni index deve stare tra 0 e 9. Poretrebbero stare tra [-infinito, +infinito] tipo per gestire l istruzione acse: exists i,j,k (a[10000*j-100*i + -k*i]>0 && b[j]==1 && c[k]==2). le condizione di fine sono quindi greater_than_array_size o not tra [0,9]*/
		reg = ((id_exist_info *)LDATA(p))->index_reg;
		gen_addi_instruction(program, reg, reg, 1);
		gen_subi_instruction(program, getNewRegister(program), reg, 10);
		gen_blt_instruction(program, stop_incremento, 0);
		prev =p;
		/*azzero tutti i precedenti index*/
		while(prev){
			regPrev = ((id_exist_info *)LDATA(p))->index_reg;
			gen_addi_instruction(program, regPrev, REG_0, 0);
			prev = prev->next;
		}
		prev =p->prev;
		/*increment il prossimo index di 1*/
		if(prev){
		regPrev = ((id_exist_info *)LDATA(prev))->index_reg;
		gen_addi_instruction(program, regPrev, regPrev, 1);
		gen_bt_instruction(program, riprendi_incremento_dal_primo_index, 0);
		}

		p = p->prev;
	}

	/*tutte le n-tuple possibile sono state provate*/
	tutte_le_tuple_non_sono_state_provate = gen_load_immediate(program, 0);

	assignLabel(program, stop_incremento);

	// Define conditions to stop iteration
   t_axe_expression is_exp_zero = handle_binary_comparison (program, $6, create_expression(0, IMMEDIATE), _EQ_);
  	t_axe_expression is_not_greater_than_array_size_ex = handle_binary_comparison (program, create_expression(greater_than_array_size, REGISTER), 		create_expression(1, IMMEDIATE), _NOTEQ_);
   t_axe_expression visit_is_not_completed = create_expression(tutte_le_tuple_non_sono_state_provate, REGISTER);

   	handle_bin_numeric_op(program, is_exp_zero, handle_bin_numeric_op(program, is_not_greater_than_array_size_ex, visit_is_not_completed, ANDB), ANDB);
   	// if (is_exp_zero AND visit_is_not_completed AND is_not_greater_than_array_size) then jump to $1
   	// else exit
   	gen_bne_instruction(program, $1, 0);
   ...
   // code for current EXISTS è gia generato: lo cancello.
	while(exist) {printf(" while(exist) ");
...
list_ids: list_ids COMMA IDENTIFIER {printf(" listIDs ");printf(" listIDsbefored ");

		id_exist_info *idExistInfo = (id_exist_info *)malloc(sizeof(id_exist_info));printf(" listIDsbefore ");
	  	idExistInfo->id = strdup($3);
	   	idExistInfo->index_reg = gen_load_immediate(program, 0);
	 	exist = addFirst(exist, idExistInfo); 

	printf(" listIDs ");} | IDENTIFIER {printf(" listID ");
