...
t_list *exist=NULL;
int array_size_reg;
int array_size_set_reg;
int count_reg;
int size;
t_axe_label *begin_increment = NULL;
...
list_ids: list_ids COMMA IDENTIFIER { printf(" ist_ids COMMA ");
	 ...
	 | IDENTIFIER { printf(" _id COMMA ");
		...
		array_size_reg = gen_load_immediate(program, 0);

		array_size_set_reg = gen_load_immediate(program, 0);
		
		count_reg = gen_load_immediate(program, 0);
		
		begin_increment = newLabel(program);		
...
exp: NUMBER      { $$ = create_expression ($1, IMMEDIATE); }
   | IDENTIFIER  {
			...
			char *i = NULL; 
				...
				while(iei){ printf(" iei ");

				     i = ((id_exist_info *)LDATA(iei))->id;

				     if (!strcmp(i, $1)){ printf(" !strcmp(i, $1) ");
   ...
   | IDENTIFIER {
	   ...
	   t_axe_label *array_size_setted = newLabel(program);
	   gen_subi_instruction(program, getNewRegister(program), array_size_set_reg, 1);
	   gen_beq_instruction(program, array_size_setted, 0);
	   gen_addi_instruction(program, array_size_reg,  REG_0, getVariable(program, $1)->arraySize);
	   gen_addi_instruction(program, array_size_set_reg,  REG_0, 1);
	   assignLabel(program, array_size_setted);
	   size = getVariable(program, $1)->arraySize;
                      ...
                      * into `arrayElement' */
                     reg = loadArrayElement(program, $1, $4);
  ...
  | EXISTS list_ids { 
	...
	LPAR exp RPAR {
		...
		// Increment the index
		assignLabel(program, begin_increment);
		gen_addi_instruction(program, count_reg, count_reg, 1);
		
		t_list *p = getLastElement(exist);
		int pos=0;
		int temp = gen_load_immediate(program, 1);
		int temp2 = gen_load_immediate(program, 0);
		int countDiAppoggio = gen_load_immediate(program, 0);
		
		gen_add_instruction(program, countDiAppoggio, countDiAppoggio, count_reg, 0);
		/*scriviamo count_reg su base arraySize ed estriamo gli index. index_reg = reg - ( reg/base^(pos+1) )*base^(pos+1) )*/
		while(p){
			id_exist_info *idExistInfo = ((id_exist_info *)LDATA(p));
			
			for(int i=0;i<pos+1;i++){gen_mul_instruction(program, temp, array_size_reg, temp, 0);}
			gen_div_instruction(program, temp2, countDiAppoggio, temp, 0);
			gen_mul_instruction(program, temp, temp2, temp, 0);
			gen_sub_instruction(program, idExistInfo->index_reg, countDiAppoggio, temp, 0);

			gen_sub_instruction(program, countDiAppoggio, countDiAppoggio, idExistInfo->index_reg, 0);
			if(pos){
				temp = gen_load_immediate(program, 1);
				for(int j=0;j<pos;j++){gen_muli_instruction(program, temp, array_size_reg,1);}			            					gen_div_instruction(program, idExistInfo->index_reg, idExistInfo->index_reg, temp, 0); 
			}
			gen_addi_instruction(program, temp, REG_0, 1);
			gen_addi_instruction(program, temp2, REG_0, 0);
			pos++;
			p = p->prev;

		}
		/*calcolo max number = (arraySize^list_lenght) - 1*/
		p = exist;
		int maxNumber = 1;
		while(p){
			maxNumber = size*maxNumber;
			p = p->next;
		}
		maxNumber = maxNumber;
   	...
   	t_axe_expression is_visit_not_completed = handle_binary_comparison (program, create_expression(count_reg, REGISTER), create_expression(maxNumber, IMMEDIATE), _LT_);
